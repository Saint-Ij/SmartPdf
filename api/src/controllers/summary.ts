...
router.get("/document/:docId", authenticate, async (req: AuthRequest, res: Response) => {
  const docs: any[] = await db.execute(sql`
    SELECT * FROM documents WHERE id = ${req.params.docId as string}::uuid AND user_id = ${req.userId!} LIMIT 1
  `);
  if (!docs[0]) throw new AppError(404, "Document not found");

  const existing = await db.execute(sql`
    SELECT * FROM summaries WHERE document_id = ${req.params.docId as string}::uuid ORDER BY created_at
  `);
  ...
  const content = await summarizeDocument(req.params.docId as string);

  const summaries = await db.execute(sql`
    INSERT INTO summaries (document_id, content)
    VALUES (${req.params.docId as string}::uuid, ${content})
    RETURNING *
  `);
