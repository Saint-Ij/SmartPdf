...
router.get("/all", authenticate, async (req: AuthRequest, res: Response) => {
  const qs = await db.execute(sql`
    SELECT q.*, d.title as document_title, s.title as section_title FROM quizzes q
...
router.post("/generate", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const { questions } = await generateQuizFromDocument(parsed.data.documentId, parsed.data.totalQuestions, parsed.data.sectionId);

  const quizzes: any[] = await db.execute(sql`
...
router.get("/:id", authenticate, async (req: AuthRequest, res: Response) => {
  const quizzes: any[] = await db.execute(sql`
    ...
    JOIN documents d ON d.id = q.document_id
    WHERE q.id = ${req.params.id as string}::uuid AND d.user_id = ${req.userId!} LIMIT 1
  `);
  if (!quizzes[0]) throw new AppError(404, "Quiz not found");
...
router.get("/:id/questions", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const questions = await db.execute(sql`
    ...
    FROM quiz_questions WHERE quiz_id = ${req.params.id as string}::uuid ORDER BY "order"
  `);
  res.json({ questions });
});
...
router.post("/:id/submit", authenticate, async (req: AuthRequest, res: Response) => {
  ...
  const quizzes: any[] = await db.execute(sql`
    SELECT * FROM quizzes WHERE id = ${req.params.id as string}::uuid LIMIT 1
  `);
  const quiz = quizzes[0];
  if (!quiz) throw new AppError(404, "Quiz not found");

  const allQuestions: any[] = await db.execute(sql`
    SELECT * FROM quiz_questions WHERE quiz_id = ${req.params.id as string}::uuid ORDER BY "order"
  `);

  let correct = 0;
  for (const q of allQuestions) {
    const userAnswer = (parsed.data.answers as Record<string, string>)[q.id as string];
    if (userAnswer && userAnswer.trim().toUpperCase() === (q.correct_answer as string).trim().toUpperCase()) {
      correct++;
    }
  }
  ...
  const attempts: any[] = await db.execute(sql`
    INSERT INTO quiz_attempts (quiz_id, user_id, score, passed, completed_at)
    VALUES (${req.params.id as string}::uuid, ${req.userId!}::uuid, ${score}, ${passed}, NOW())
    RETURNING *
  `);

  await db.execute(sql`
    UPDATE quizzes SET status = ${passed ? "passed" : "failed"} WHERE id = ${req.params.id as string}::uuid
  `);

  res.json({ attempt: attempts[0], score, passed, correct, total });
});

router.get("/:id/answers", authenticate, async (req: AuthRequest, res: Response) => {
  const questions = await db.execute(sql`
    SELECT * FROM quiz_questions WHERE quiz_id = ${req.params.id as string}::uuid ORDER BY "order"
  `);
  res.json({ questions });
