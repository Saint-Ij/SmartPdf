...
async function callLLM(messages: { role: string; content: string }[]): Promise<{
  content: string;
...
export async function chatWithDocument(
  ...
  const contextText = (similarChunks as any[]).map((c: any) => c.content).join("\n\n---\n\n");
  const sourceIds = (similarChunks as any[]).map((c: any) => c.id);
...
${contextText || "No relevant context found."}`;
  ...
  const llmMessages = [
    ...
    ...(recentMessages as any[]).map((m: any) => ({ role: m.role, content: m.content })),
...
export async function generateQuizFromDocument(
  ...
  sectionId?: string,
): Promise<{ questions: { question: string; options: string[]; correctAnswer: string; explanation: string }[] }> {
  const docChunks = await db.execute(sectionId
    ? sql`SELECT content FROM chunks WHERE document_id = ${documentId}::uuid AND section_id = ${sectionId}::uuid LIMIT 20`
    : sql`SELECT content FROM chunks WHERE document_id = ${documentId}::uuid LIMIT 20`
  );

  const contextText = (docChunks as any[]).map((c: any) => c.content).join("\n\n").slice(0, 8000);
...
export async function generateFlashcardsFromDocument(
  ...
  count: number = 5,
  sectionId?: string,
): Promise<{ flashcards: { front: string; back: string }[] }> {
  const docChunks = await db.execute(sectionId
    ? sql`SELECT content FROM chunks WHERE document_id = ${documentId}::uuid AND section_id = ${sectionId}::uuid LIMIT 20`
    : sql`SELECT content FROM chunks WHERE document_id = ${documentId}::uuid LIMIT 20`
  );
